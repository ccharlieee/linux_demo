	...
	if(data == '0')
		{
            ...
            printk(KERN_WARNING "0000000%s\n",__FUNCTION__);
		}
	else
		{
			gpio_set_value(led_gpio, 0);
            printk(KERN_WARNING "11111111%s\n",__FUNCTION__);
		}	
...
static int __init demo_module_init(void)
{
	...
	demo_device = device_create(demo_class,NULL,MKDEV(major,0),NULL,"gpio_beep");
	if(IS_ERR(demo_device)){
		ret = PTR_ERR(demo_device);
		goto device_create_fail;
	}

    nd = of_find_node_by_path("/beep");
    if(nd == NULL)
    {
        ret = -EINVAL;
        goto fail_findnode;
